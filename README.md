...
- Автообновление в моем исполнении работает следующим образом, когда я вношу изменения в файл-скрипт, расположенный у меня в приватном репозитории на GitHub, то его хэш сумма md5 меняется, далее код, который я встроил в репозиторий, проверяет раз в сутки в 3:00, обнаружив, что хэш сумма md5 изменилась, начинает процесс обновления немедленно. Правило проверки выбрано каждый день в 3:00 ночи, если же изменений у меня на GitHub в отношении этого файла не было, то хэш сумма остаётся прежней, ну и соответственно ничего не обновляется. В индивидуальных случаях потребуется разовая ссылку на патч.
...
2. Вставить:
```bash
rutracker.org instagram.com facebook.com x.com scontent-hel3-1.xx.fbcdn.net scontent-hel3-1.cdninstagram.com rr13---sn-n8v7kn7l.googlevideo.com
```
...
### Можно приступать к установке репозитория [Zapret](https://github.com/bol-van/zapret) или написать в группу, чтобы использовать собственный коммерческий проект с автообновлением.
